...
function UserPost() {
  ...
  const handlePageClick = (event) => {
    const newOffset = (event.selected * itemsPerPage) % data?.post?.length;
    console.log(
      ...
      <section className="px-2 mt-8  md:mt-16 lg:px-12 md:px-12">
        <div className="flex flex-col gap-2 w-fit">
          ...
          <span className="text-lg font-light uppercase text-purple">
            {login?.firstname} / Posts
          </span>
        </div>
      </section>
      {/* post pagination section */}
      <section className="px-2 mt-8 lg:px-12 md:px-12">
        {data?.post.length <= 0 ? (
          <div className="flex justify-center md:text-3xl">
            <div className="flex flex-col items-center ">
              <p className="text-sm text-gray-400">No post found</p>
              <img src={Nopostimg} alt="no post found" className="w-20 h-20" />
            </div>
          </div>
        ) : (
